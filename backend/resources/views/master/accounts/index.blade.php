...
<div class="flex items-center justify-between mb-4">
  <h2 class="text-xl font-semibold">Master Akun Kas/Bank</h2>
  <a href="{{ route('master.accounts.create') }}" class="bg-blue-600 hover:bg-blue-700 text-white px-3 py-2 rounded">Tambah</a>
</div>

<form method="GET" class="mb-4 flex gap-2 flex-wrap">
  <input name="search" value="{{ request('search') }}" placeholder="Cari kode/nama/no rekening" class="border rounded px-3 py-2 w-64" />
  <select name="type" class="border rounded px-3 py-2">
    <option value="">Semua</option>
    <option value="cash" @selected(request('type')==='cash')>Kas</option>
    <option value="bank" @selected(request('type')==='bank')>Bank</option>
  </select>
  <button class="px-3 py-2 bg-gray-100 rounded">Filter</button>
  <a href="{{ route('master.accounts.index') }}" class="px-3 py-2 text-sm underline">Reset</a>
</form>
      ...
      <tr class="border-t">
        ...
        <td class="py-2 px-3">
          <div class="flex gap-2">
            <a class="px-2 py-1 bg-gray-100 rounded" href="{{ route('master.accounts.edit',$r) }}">Ubah</a>
            <form method="POST" action="{{ route('master.accounts.destroy',$r) }}" onsubmit="return confirm('Hapus akun?')">
              @csrf @method('DELETE')
