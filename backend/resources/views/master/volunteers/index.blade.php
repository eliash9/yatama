...
<div class="flex items-center justify-between mb-4">
  <h2 class="text-xl font-semibold">Master Relawan</h2>
  <a href="{{ route('master.volunteers.create') }}" class="bg-blue-600 hover:bg-blue-700 text-white px-3 py-2 rounded">Tambah</a>
</div>
...
<form method="GET" class="mb-4 flex gap-2">
  ...
  <a href="{{ route('master.volunteers.index') }}" class="px-3 py-2 text-sm underline">Reset</a>
</form>
  ...
  <table class="min-w-full text-sm">
    <thead class="text-gray-600">
      <tr class="text-left">
        ...
        <th class="py-2 px-3">Keahlian</th>
    ...
    <tbody>
      ...
      <tr class="border-t">
        ...
        <td class="py-2 px-3">{{ $r->skills }}</td>
        <td class="py-2 px-3">{!! $r->is_active ? '<span class="text-green-600">Aktif</span>' : '<span class="text-gray-500">Nonaktif</span>' !!}</td>
        <td class="py-2 px-3">
          <div class="flex gap-2">
            <a class="px-2 py-1 bg-gray-100 rounded" href="{{ route('master.volunteers.edit',$r) }}">Ubah</a>
            <form method="POST" action="{{ route('master.volunteers.destroy',$r) }}" onsubmit="return confirm('Hapus data ini?')">
              @csrf @method('DELETE')
